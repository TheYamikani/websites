...
import { HOSTS, SITE } from '@/lib/content'
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="md:col-span-9">
            <h1 className="display font-semibold text-4xl sm:text-5xl md:text-6xl lg:text-[88px] leading-[1.0] tracking-[-0.04em] text-paper max-w-5xl">
              {SITE.name} is the more serious network wrapper around the reporting and commentary operation that was already proving the demand.
            </h1>
      ...
      <section className="container-edge pb-24 md:pb-32 grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-16">
        <div className="md:col-span-5 md:sticky md:top-28 md:self-start">
          <figure className="relative">
            <div className="aspect-[4/5] w-full bg-bone overflow-hidden border border-paper/10 shadow-[0_30px_80px_rgba(0,0,0,0.45)] rounded-[30px]">
              <img
                src="/zbn-logo.png"
                alt={SITE.name}
            ...
            <figcaption className="mt-4 flex justify-between font-mono text-[11px] uppercase tracking-[0.18em] text-paper/60">
              ...
              <span className="text-signal">{SITE.brandMark}</span>
            </figcaption>
          </figure>
          ...
          <dl className="mt-10 space-y-6 font-mono text-xs uppercase tracking-[0.14em]">
            ...
            <div>
              ...
              <dd className="text-paper text-sm">{SITE.network}</dd>
            </div>
            <div>
        ...
        <div className="md:col-span-7 space-y-6 text-lg md:text-[21px] leading-[1.5] text-paper/85 tracking-[-0.003em]">
          ...
          <div className="pt-12 grid grid-cols-1 sm:grid-cols-2 gap-10">
            ...
            <div>
              <p className="eyebrow mb-6">Build Notes</p>
              <ul className="space-y-0">
          ...
          <div className="pt-8">
            <p className="eyebrow mb-6">Founding Hosts</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {HOSTS.map((host) => (
                <article key={host.name} className="glass-panel rounded-[26px] p-5">
                  <img
                    src={host.image}
                    alt={host.name}
                    className="w-full aspect-[4/5] object-cover object-top rounded-[20px] border border-paper/10"
                  />
                  <div className="mt-4">
                    <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-signal mb-2">
                      {host.role}
                    </p>
                    <h2 className="text-paper text-2xl font-semibold tracking-[-0.02em]">{host.name}</h2>
                    <p className="text-paper/60 text-sm mt-1">{host.base}</p>
                    <p className="text-paper/76 text-sm leading-relaxed mt-4">{host.blurb}</p>
                  </div>
                </article>
              ))}
            </div>
          ...
          <div className="pt-12">
            <Link
              to="/work"
              className="inline-flex items-center gap-3 bg-signal text-ink px-6 py-4 text-[12px] uppercase tracking-[0.2em] font-semibold hover:bg-paper hover:text-ink transition-colors"
            >
