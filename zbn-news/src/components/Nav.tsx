...
export default function Nav() {
  ...
  return (
    ...
    >
        ...
        >
          {logoOk ? (
            <img
              ...
              className="w-12 h-12 md:w-16 md:h-16 object-cover object-center rounded-[14px] border border-paper/10 shadow-[0_0_0_1px_rgba(255,255,255,0.05)]"
            />
          ) : (
            <span className="inline-flex items-center justify-center w-9 h-9 bg-signal text-ink font-mono text-[11px] font-bold rounded-xl">
              {SITE.brandMark}
            </span>
          )}
          <span className="hidden sm:block">
            <span className="block font-sans text-[16px] md:text-[18px] font-semibold tracking-tight leading-none text-paper">
              {SITE.name}
            </span>
            <span className="block font-mono text-[10px] uppercase tracking-[0.22em] text-signal/90 mt-1">
              Zero Boundary Network
            </span>
          </span>
        </Link>
      ...
      {open && (
        <nav className="md:hidden bg-[#07111D] text-paper border-t border-paper/10">
